  ...
  return {
    ...
    buildReviewTable: function() {
      ...
      templateData = Object.keys(answers).map(function(questionId) {
        ...
        var correctAnswer = question.answers.filter(function(answer) {
    ...
    getScore: function() {
      ...
      var correctlyAnswers = Object.keys(answers).forEach(function(answer) {
...
$(document).ready(function() {
  ...
  $('.js-qz-share-yes').on('click', function() {
    ...
    $q.find('h2').text(
      Handlebars.compile($q.find('h2').text())(game.getScore())
    );

    $q.find('p').text(Handlebars.compile($q.find('p').text())(game.getScore()));
    ...
    data.forEach(function(field) {
      ...
      if (field.name === 'email' && field.value.length) {
        if (!re.test(field.value))
          errors.push({
            name: 'email',
            message: 'Email address is incorrectly formatted',
          });
      }
    });
  ...
  $('.js-qz-share-dialog').on('click', function() {
    ...
    var description = Handlebars.compile($q.data('description').trim())(
      game.getScore()
    );
    ...
    window.open(
      'https://facebook.com/sharer.php?u=' + shareUrl,
      '_blank',
